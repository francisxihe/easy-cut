...
function registerPlugin() {
  ...
  if (pluginPath) ipcRenderer.send("addPluginFromPackage", pluginPath[0]);
}
...
function addFilter(filter) {
  if (timeline.getSelection()[0]) {
    ...
    old.push(filter);
    timeline.changeItem(timeline.getSelection()[0].row, { filters: old });
    updateInspect();
  }
}
...
function updateInspect() {
  ...
  } else inspectorManager.properties = {};
}
