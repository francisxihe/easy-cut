var timeline = new links.Timeline(document.getElementById("timelinecont"), {
  zoomMax: 3600000,
...
function ondelete() {}
function onchanged() {
  if (!liveCheckValidLength()) timeline.cancelChange();
}
function onselect() {
  updateInspect();
}
function onchange() {
  ...
  else timeline.changeItem(timeline.getSelection()[0].row, { valid: false });
}
...
function split() {
  if (timeline.getSelection()[0]) {
    ...
    timeline.changeItem(timeline.getSelection()[0].row, { end: customTime });
  }
}
...
function liveCheckValidLength() {
  ...
  if ((clip.end - clip.start) / 1000 + seek <= clip.maxduration) return true;
  return false;
}

function tlCompare(a, b) {
...
function getEndTime() {
  ...
  for (var i = 0; i < tldata.length; i++) {
    if (back(tldata[i].end) > time) time = back(tldata[i].end);
  }
  return time;
}
