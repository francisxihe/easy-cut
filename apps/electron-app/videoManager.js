...
class VideoManager {
  ...
  renderAll(index, callbacks) {
    ...
    if (index >= this.workFiles.length) return Promise.resolve(); // at the end
  ...
  stitchMaster(callbacks) {
    return new Promise((resolve, reject) => {
      ...
      var listFileName = this.wd + "/master/list.txt",
        fileNames = "";
  ...
  convertToCompliant(input, name) {
    ...
    command.input(input).save(this.wd + name + sch.format);
  }
}
